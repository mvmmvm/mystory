class StoriesController < ApplicationController
    ...
    def hoge
        ...
        names.each_with_index do |name, count|
            ...
            p fw_count
            gender = validate(story.match(/(人物(#{fw_count}|#{count})|#{name})の性別(：|:|は)\s?((?:.|\s)+?)(?=\s[^(：|:)\s]+(：|:))/m), 4)
            p gender
            personality =  validate(story.match(/(人物(#{fw_count}|#{count})|#{name})の性格(：|:|は)\s?((?:.|\s)+?)(?=\s[^(：|:)\s]+(：|:))/m), 4)
            p personality
            job = validate(story.match(/(人物(#{fw_count}|#{count})|#{name})の職業(：|:|は)\s?((?:.|\s)+?)(?=\s[^(：|:)\s]+(：|:))/m), 4)
            p job
            introduce = validate(story.match(/(人物(#{fw_count}|#{count})|#{name})のここにいる理由と自身の秘密の内容と事件直前に取った行動を独白させてください(：|:)\s?((?:.|\s)+?)(?=\s[^(：|:)\s]+(：|:))/m), 4)
            p introduce
            stuff = validate(story.match(/(人物(#{fw_count}|#{count})|#{name})の秘密の証拠品(：|:|は)\s?((?:.|\s)+?)(?=\s[^(：|:)\s]+(：|:))/m), 4)
            p stuff
            evidence = [
                validate(story.match(/(人物(#{fw_count}|#{count})|#{name})が事件後に他の人物に聞いたり現場を調べてわかった.+?(１|1)を独白させてください(：|:)\s?((?:.|\s)+?)(?=\s[^(：|:)\s]+(：|:))/m), 5),
                validate(story.match(/(人物(#{fw_count}|#{count})|#{name})が事件後に他の人物に聞いたり現場を調べてわかった.+?(２|2)を独白させてください(：|:)\s?((?:.|\s)+?)(?=\s[^(：|:)\s]+(：|:))/m), 5),
                validate(story.match(/(人物(#{fw_count}|#{count})|#{name})が事件後に他の人物に聞いたり現場を調べてわかった.+?(３|3)を独白させてください(：|:)\s?((?:.|\s)+?)(?=\s[^(：|:)\s]+(：|:))/m), 5)
            ]
            p evidence
            p reason
            p criminal
