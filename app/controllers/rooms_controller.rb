class RoomsController < ApplicationController
    ...
    def request_gpt(names, genders)
        ...
        main_str =
            ...
            "#{joined_names}のうちの1人が高橋信夫を殺害した犯人です。"\
        ...
        names.zip(genders) do |name, gender|
            character_str +=
                ...
                "#{name}の性格:\n"\
                "#{name}の職業:\n"\
                "#{name}のここにいる理由と、自身の秘密の内容と、秘密を隠すため、事件直前に取った行動を独白させてください:「」\n"\
                "#{name}の秘密の種類:\n"\
                "#{name}の秘密の証拠品:\n"\
        ...
        names.each do |name|
            character_reason_str +=
            "#{name}の行動が他の人物から言及されている件について、理由を独白させてください:「」\n"
        end
